...
\\
Having studied enough from Related Work (which was mentioned to a great extent above) in the field of the Opinion Mining, it was clear that in order to extract the arguments from the texts that we had available we would have to apply some Machine Learning process. Therefore, it is now clearer that at this step (\ref{321_ref}), we had to define some parameters which will define with a relative clarity whether a sentence is an argument or not.\\
\\
